...
export const registerStation = async (req, res) => {
  ...
  try {
    ...
    if (usedLocation) {
      return res.status(400).json({ message: "Location already registered" });
    }
...
export const loginStation = async (req, res) => {
  ...
  try {
    ...
    if (!loginUser) {
      return res
        .status(301)
        .json({ message: "Station with that email does not exists" });
    }
...
export const getStationWithId = async (req, res) => {
  ...
  try {
    ...
    if (!station) {
      return res
        .status(400)
        .json({ message: "Station with that id does not exists" });
    }
